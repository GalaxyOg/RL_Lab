...
MAZE_W = 2  # grid width
# define reward
normal_step_reward = 0
keep_step_reward = 0  # 保持在非forbidden aero
forbidden_step_reward = -1
boundary_step_reward = -1
target_step_reward = 1
...
class Maze(tk.Tk, object):
    ...
    policy_state_action = np.concatenate(
        (np.ones([MAZE_H * MAZE_W, 1], dtype=float), np.zeros([MAZE_H * MAZE_W, 4], dtype=float)), 1)
...
if __name__ == '__main__':
    ...
    # env.reset([2, 2])
    # print(env.reset(s))
    # print(env.state_value[1, 0], type(env.state_value[1, 0]))
